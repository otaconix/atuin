...
pub struct Search {
    all_history: Vec<(History, nucleo_matcher::Utf32String, i32)>,
    engine: nucleo_matcher::Matcher,
...
impl Search {
    pub fn new() -> Self {
        Search {
            all_history: vec![],
            engine: nucleo_matcher::Matcher::default(),
            pattern: nucleo_matcher::pattern::Pattern::default(),
        }
    }
}
    ...
    async fn full_query(
        ...
        if self.all_history.is_empty() {
            self.all_history = db
                ...
                .into_iter()
                .map(|(history, count)| {
                    let utf32_command = nucleo_matcher::Utf32String::from(history.command.clone());
                    (history, utf32_command, count)
                })
                .collect();
        }

        Ok(self.fuzzy_search(state).await)
    }
}

impl Search {
    async fn fuzzy_search(&mut self, state: &SearchState) -> Vec<History> {
        ...
        for (i, (history, command, count)) in self.all_history.iter().enumerate() {
            if i % 256 == 0 {
                yield_now().await;
            }
            ...
            if let Some(score) =
                self.pattern
                    .indices(command.slice(..), &mut self.engine, &mut indices)
            {
